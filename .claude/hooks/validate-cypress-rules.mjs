...
import { containsAcceptedTicket } from './lib/task-protocol.mjs';
...
if (isSpec && isSmokePath(filePath)) {
  ...
  if (QUARANTINE_TAG_RE.test(content)) {
    const hasTicket = containsAcceptedTicket(content, loadHarnessConfig().atlassian?.projectKeys);
  ...
  if (/\b(?:describe|it|context)\.skip\s*\(/.test(content) && !containsAcceptedTicket(content, loadHarnessConfig().atlassian?.projectKeys))
    warnings.push(
      'Skipped block without a FirstHelp ticket reference (SERV, GEARS, LOS, or SDX) — a dead module hides inside a healthy ' +
