...
- Which module? (match to a YAML file in `Test-Case-Automation-Using-Claude-Agents/`)
...
1. **`Test-Case-Automation-Using-Claude-Agents/<module>.yaml`** — OpenAPI spec. Note: `status: draft`
   specs have many TBDs; treat unresolved items as gaps, not facts.
...
2. `confluence_get_page_outline` then `confluence_get_page_section` for the endpoints table —
   capture: method, path, query params, path params, response shape (top-level fields + types),
...
4. If the user provides a Jira ticket key, pull it with `get_issue` for acceptance criteria or
   known gaps.
...
|--------|-------------|--------|
| Spec | Test-Case-Automation-Using-Claude-Agents/<file>.yaml | confirmed / draft |
...
cases were generated, and what the Known Gaps are. Done — no code is written, no `ExitPlanMode`.
...
- Do not call `EnterPlanMode` — this skill produces a document, not a code implementation plan.
