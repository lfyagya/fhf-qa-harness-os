...
## FHF Cypress work — conventions only, no spec writes

Before anything else, check whether this is FHF QA automation (this workspace, E2E, Smoke,
dashboards), including a generic "write a test" request when project context is FHF.

If it is FHF:

1. Do **not** Write, Edit, or create spec, command, or Cypress config files.
2. You **may** read `author.md` / `references/author/*` and return Cypress-native conventions
   (selectors, retries not `cy.wait(n)`, intercepts, independent tests) as input for the generator.
3. Do not proceed to Execute. Do not emit a complete spec, a command-first lookalike, or a
   "generated by Cypress Author" file. That was the ADR-0004 failure mode.
4. Stop so the parent can spawn `cypress-generator` (`new-test` route). `cypress-gate` reviews
   that output.

If it is clearly a different repo/project, this note does not apply — continue below as normal.
...
Only reached when the FHF convention-only guard above did not apply. When moving to [author.md](./author.md),
