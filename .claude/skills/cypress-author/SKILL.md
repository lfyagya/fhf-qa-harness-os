...
0. **FHF guard** — If this is FHF QA automation (E2E, Smoke, dashboards, this workspace): do **not**
   Write, Edit, or create spec/command/config files. You may read this skill's author rules only as
   Cypress-native conventions. Then stop so the parent can spawn `cypress-generator`. Never produce
   a plausible FHF spec or a "generated by Cypress Author" file. `cypress-gate` still reviews generator output.
