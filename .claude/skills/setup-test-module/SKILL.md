...
**Description**: 
Automates creation of a new test module with complete boilerplate code. Generates an API client, conftest.py with required fixtures (schema, state manager, API client), data builder factory, test file template, and optional updates to db_schema.py and pytest.ini. All generated files follow project conventions and are immediately usable—users add endpoint methods, factory implementations, and test cases after reviewing the setup structure.
...
**Step 7.3: Update tests/.env** (if needed)
- Add endpoint env vars required by the module:
  ```env
...
📝 Next Steps:
  ...
  9. [ ] Add module to pytest.ini file_order (if top-level module)
```
