...
3. `engineering.harness.hooks` stays the only hook list. `.codex/hooks.json` uses the same commands as Claude. Codex has no `PostToolUseFailure` event and reports a failed tool on `PostToolUse`, so `failure-loop-guard.mjs` runs on `PostToolUse` for every host and still runs on `PostToolUseFailure` where that event exists. A successful result does not count. `engineering.harness.adapters.codex.hookCapability` is `hooks-json`. `instructionFile` stays `AGENTS.md`. The hook command string is the same on Claude, Cursor, and Codex. The emitter keeps `user_message` and `additional_context` unless the payload already carries `turn_id` or `permission_mode`, the Codex wire that rejects those fields. It never sends `continue` on `PreToolUse`. Subagent scope reads `agent_type` or `subagent_type` and treats `generalPurpose` as `general-purpose`. ADR-0041 lets general-purpose and explore start inside the one sprint task. The launcher resolves the git root when `CLAUDE_PROJECT_DIR` and `CURSOR_PROJECT_DIR` are unset. Lane repositories do not receive a second hook file (ADR-0032). The tool-name matcher is one string, the union of the names Claude, Cursor, and Codex use for that action, so the same guard fires in each tool. Empty `.codex/` directories on those lanes are still removed.
