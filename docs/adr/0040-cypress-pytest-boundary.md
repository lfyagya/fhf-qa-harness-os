...
2. The sprint task picks the path. `new-test` routes to Cypress and the `frontend-change` bundle. `backend-test` and `cross-layer-test-generation` route to one `qa-automation-generator`. The prompt states the boundary. A backend write or pytest searches `.harness/tasks`. One manifest with a SERV ticket is the task. Several are named so the engineer can choose. An empty directory gets `sprint-task.json`, and the hook asks for the SERV ticket, module, and the pytest remainder. Routing continues. The write or pytest waits until that task records the selected path and a non-production environment.
