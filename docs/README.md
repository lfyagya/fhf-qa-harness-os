...
| Smoke UI → API → DB chain coverage, per sub-module (2026-08-20) | `planning/smoke-ui-api-db-chain-coverage.md` — **temporary stand-in**: belongs in `planning/coverage/` and should be folded into the matrix above. Nothing in this repository blocks that; the earlier note blamed a deny rule, which is a machine-local tool permission and misfires only on relative paths (verified 2026-09-20) |
