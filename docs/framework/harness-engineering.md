...
- `engineering.harness.hooks` is the only hook list. Claude settings, Cursor hooks, and `.codex/hooks.json` are projections of it. Each action uses one matcher string, the union of the tool names those hosts use. Session start and `AGENTS.md` carry the same `rules/` index, so every tool sees one standing-order list.
