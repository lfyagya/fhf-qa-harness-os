...
argument-hint: "Cypress command, concept, or version question"
metadata:
  version: 1.1.0
---
...
If the user only needs **writing or fixing tests** without a documentation lookup, use the `cypress-generator` / `cypress-debugger` agents; if they only need **test explanation** without fetching docs, prefer `cypress-explain`. Use **this** skill when official documentation is the source of truth.
