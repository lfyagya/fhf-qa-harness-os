...
Use on the E2E lane (Dev/QA). On the smoke lane the app under test is production: `dom`, `aria`,
`inspect`, and `pin` return live customer data, so use tap there only with the owner's explicit
prod-data opt-in for the session (`.claude/rules/prod-data-handling.md`). Smoke stays GET-only.
New or changed specs still follow `.claude/rules/cypress-standards.md` and go through
`cypress-gate`.
