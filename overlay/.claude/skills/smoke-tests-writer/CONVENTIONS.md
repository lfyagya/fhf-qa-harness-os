...
from tests.commons.assertions import (
    assert_table_existence, assert_query_results_found, assert_truthy,
)
...
from tests.conftest import BaseDB
from tests.commons.assertions import (
    assert_set_matches_expected, assert_required_fields_not_null, assert_value_equals,
)
...
def test_fn_verify_queue_distribution_is_callable(self, region):
    ...
    assert_truthy(results, f"fn_verify_queue_distribution returned no rows for <DEPT>/{region}.")
    failures = [
...
def test_expected_foo_rows_exist(self, table, key_col, name_col, expected):
    ...
    with allure.step(f"Validate {name_col} and IS_ACTIVE=1 for each expected row"):
        ...
        for key, expected_name in expected.items():
            row = by_key[key]
            assert_value_equals(row[name_col], expected_name, f"{name_col} for {key}")
            assert_value_equals(int(row["IS_ACTIVE"]), 1, f"IS_ACTIVE for {key}")
...
### Fixtures in `tests/smoke/conftest.py` (registry — never a hand-written fixture body)
```python
from api.foo_client import FooClient          # with the other client imports

foo_api_client    = _client("foo_api_client", FooClient)
foo_state_manager = _state("foo_state_manager")
```
