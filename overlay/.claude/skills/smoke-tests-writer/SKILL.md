...
description: Code-only backend smoke test writer for fhf-backend-automation/tests/smoke. Takes a specification from smoke-test-cases and produces the GET-only smoke implementation. Use after the specification exists.
---
...
all facts come from the spec.** Plans, gets approval, then writes and verifies. Backend smoke
runs against production — the agent writes and collects it, the owner runs it
(`.claude/rules/prod-data-handling.md`).
...
4. **No code before plan approval** (the approved GSD plan, or the plan shown for a `/gsd-quick`
   request).
5. **Smoke = read-only, no CRUD.** Only GET methods and SELECT queries; no create/update/delete tests. If the spec lists a non-GET endpoint, skip it with a
   comment. If the user asks to add one, refuse and explain why.
6. **Verify by collecting.** Show actual `py_compile` and `pytest --collect-only` output before
   declaring done. Do not execute the smoke tests yourself.
...
Phases 3–4 are design; write nothing yet.
...
- Registry line in `tests/smoke/conftest.py`
...
## Phase 4 — Write the plan, get approval
...
| tests/smoke/conftest.py | MODIFY | add <X> `_client(...)` / `_state(...)` registry lines |
...
5. New `_client(...)` / `_state(...)` registry lines in `tests/smoke/conftest.py`
