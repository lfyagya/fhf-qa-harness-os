...
**Do NOT use this skill when:** The user states they are not asking about Cypress, when the user mentions an alternative testing tool without referencing Cypress, when the primary ask is to create, fix, update, or run tests, or when the user actually wants a merge-readiness verdict (PASS/BLOCK against FHF's architecture/compliance rules) rather than a generic walkthrough or critique — that's `cypress-gate`, not this skill. This skill has no FHF-specific rule knowledge; it critiques against Cypress best practices only.
