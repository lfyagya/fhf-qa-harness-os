...
✓ Name the test for TestRail with `@allure.title(...)` / `allure.dynamic.title(...)` — never a `[C<id>]` docstring marker  
...
```python
@pytest.mark.order(1)
@allure.title("Create new payment and verify DB state")
def test_create_payment(self, payment_processing_api_client, payment_processing_schema):
    ...
    columns, result = self.execute_query(
        f"SELECT * FROM {payment_processing_schema.table_payment_transaction} WHERE ID = :id",
        {"id": response.json()["payment_id"]},
    )
    assert_value_equals(result[0][COL_AMOUNT], 500, "payment amount")
```

## User Notes

- **Fixtures provided:** By the setup-test-module skill or a manual conftest.py
...
- **TestRail:** titles sync from `allure.dynamic.title` (`.claude/rules/testing.md`)
- **Reference tests:** tests/loss_mitigation/repo_invoice/, tests/ancillary/ancillary_cancellation_dashboard/
