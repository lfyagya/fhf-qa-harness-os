...
4. **Intent vs app source.** Check the intent against `fhf-dashboards/src`. **Necessary, but never
   ...
   defect hides.
5. **Adjudicate with the application specs.** `Test-Case-Automation-Using-Claude-Agents/specs/` —
   per-module YAML under `specs/modules/<module>/` with numbered business rules (`BR-<MOD>-NNN`);
   `specs/INDEX.md` lists each spec's maturity. This is the independent third reference: it states what the app is
...
Also standing: after 3 attempts on the same failure, stop, escalate, and prefer instrumentation
over another fix. Verify that a manual browser session and
