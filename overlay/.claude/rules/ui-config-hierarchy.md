...
Entry criteria for a new or intentionally migrated domain are in `cypress-standards.md`. Missing
selectors or state are a testability proposal, not an automation shortcut.
...
Before adding a `data-cy` literal to a `configs/ui/**` file, grep sibling configs for it (outside `common.ui.js`). `cypress-gate` also checks duplication across the two lanes (E2E vs Smoke) and CSS-class selectors.
...
Both classes are enforced by `CypressFHF/fhf-dashboards/scripts/check-alias-commands.js` in the
smoke lane, wired into `buildspec.yml` `pre_build`.
