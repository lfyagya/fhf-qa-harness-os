...
Read-only evidence source for QA. **NEVER write, edit, or create files there** — it is owned by the frontend dev team. Missing `data-cy` hook → record it as a gap (component path) and propose it upstream to the frontend team.

Use the product specs in `Test-Case-Automation-Using-Claude-Agents/specs/` once the source lookup needs product context.
...
| Mutable functional suites | `tests/{ancillary,funding,loss_mitigation,unifi}/` | these create and modify records; Dev/QA only |
...
Counts drift: treat `api/` on disk as the list of clients, `api-standards.md` as the pattern.
