...
Cypress owns the browser behavior of fhf-dashboards. This lane is the remainder: REST, service, Oracle, and Python workflow behavior that the Cypress spec for the sprint task does not already assert. The sprint task decides the path. A write or pytest searches `.harness/tasks`, uses the one FirstHelp task stored there (SERV, GEARS, LOS, or SDX), and creates `sprint-task.json` when the directory is empty. The ask is that ticket, the module, and that gap. Routing continues. A write or pytest waits until the task records the selected non-production path.
