...
4. Generate Cypress plus pytest coverage as one connected flow per change unit, bound by a named
   seam — the endpoint contract plus a correlation key both lanes carry. Two single-layer suites
   over the same acceptance criteria are not cross-layer coverage. Never edit application source.
